...
static BLAKE2_INLINE void ablake2b_set_lastargon_block(ablake2b_state *S) {
    if (S->last_node) {
...
static BLAKE2_INLINE void ablake2b_invalidate_state(ablake2b_state *S) {
    ...
    ablake2b_set_lastargon_block(S); /* invalidate for further use */
}
...
/* Sequential blake2b initialization */
 int ablake2b_init(ablake2b_state *S, size_t outlen) {
    ...
    /* Setup Parameter argon_block for unkeyed BLAKE2 */
 ...
 int ablake2b_init_key(ablake2b_state *S, size_t outlen, const void *key,
    ...
    /* Setup Parameter argon_block for keyed BLAKE2 */
    P.digest_length = (uint8_t)outlen;
    ...
    {
        uint8_t argon_block[ablake2b_argon_blockBYTES];
        memset(argon_block, 0, ablake2b_argon_blockBYTES);
        memcpy(argon_block, key, keylen);
        ablake2b_update(S, argon_block, ablake2b_argon_blockBYTES);
        /* Burn the key from stack */
        clear_internal_memory(argon_block, ablake2b_argon_blockBYTES);
    }
    return 0;
}
 ...
 static inline int ablake2b_compress(ablake2b_state *S, const uint8_t argon_block[ablake2b_argon_blockBYTES])
 {
	 __m128i row1l, row1h;
	 __m128i row2l, row2h;
	 __m128i row3l, row3h;
 ...
 static inline int ablake2b4rounds_compress(ablake2b_state *S, const uint8_t argon_block[ablake2b_argon_blockBYTES])
 {
	 __m128i row1l, row1h;
	 __m128i row2l, row2h;
	 __m128i row3l, row3h;
 ...
 void ablake2b_compress_old(ablake2b_state *S, const uint8_t *argon_block) {
...
#undef ROUND
 ...
 void ablake2b_compress_test(ablake2b_state *S, const uint8_t *argon_block) {
...
#undef ROUND
 ...
 void ablake2b4rounds_compress_old(ablake2b_state *S, const uint8_t *argon_block) {
...
#undef ROUND
 ...
 int ablake2b_update(ablake2b_state *S, const void *in, size_t inlen) {
    ...
    if (S->buflen + inlen > ablake2b_argon_blockBYTES) {
        /* Complete current argon_block */
        size_t left = S->buflen;
        size_t fill = ablake2b_argon_blockBYTES - left;
        memcpy(&S->buf[left], pin, fill);
        ablake2b_increment_counter(S, ablake2b_argon_blockBYTES);
        ablake2b_compress(S, S->buf);
        S->buflen = 0;
        inlen -= fill;
        pin += fill;
        /* Avoid buffer copies when possible */
        while (inlen > ablake2b_argon_blockBYTES) {
            ablake2b_increment_counter(S, ablake2b_argon_blockBYTES);
            ablake2b_compress(S, pin);
            inlen -= ablake2b_argon_blockBYTES;
            pin += ablake2b_argon_blockBYTES;
        }
    }
 ...
 int ablake2b_final(ablake2b_state *S, void *out, size_t outlen) {
    ...
    ablake2b_set_lastargon_block(S);
    memset(&S->buf[S->buflen], 0, ablake2b_argon_blockBYTES - S->buflen); /* Padding */
    ablake2b_compress(S, S->buf);
