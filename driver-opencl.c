...
if (gpu->algorithm.type == ALGO_NEOSCRYPT) {
  ...
  size_t worksize = globalThreads[0];
        ...
        status=clEnqueueNDRangeKernel(clState->commandQueue, clState->neoscrypt_gpu_hash_start, 1, NULL, grid2, block2, 0, NULL, NULL);
		if (status != CL_SUCCESS) {
		    printf("Error %d: clEnqueueNDRangeKernel neoscrypt_gpu_hash_start\n", status);
		    return -1;
	    }
        status=clEnqueueNDRangeKernel(clState->commandQueue, clState->neoscrypt_gpu_hash_salsa1, 2, NULL, grid3, block3, 0, NULL, NULL);
		if (status != CL_SUCCESS) {
		    printf("Error %d: clEnqueueNDRangeKernel neoscrypt_gpu_hash_salsa1\n", status);
		    return -1;
	    }
        status=clEnqueueNDRangeKernel(clState->commandQueue, clState->neoscrypt_gpu_hash_chacha1, 2, NULL, grid3, block3, 0, NULL, NULL);
		if (status != CL_SUCCESS) {
		    printf("Error %d: clEnqueueNDRangeKernel neoscrypt_gpu_hash_chacha1\n", status);
		    return -1;
	    }
        status=clEnqueueNDRangeKernel(clState->commandQueue, clState->neoscrypt_gpu_hash_ending, 1, NULL, grid2, block2, 0, NULL, NULL);
		if (status != CL_SUCCESS) {
