...
#define ARGON2D_G_SHUFFLE(a, b, c, d, m1, m2) { \
    a = a + b + m1; \
    d = ROTR64(d ^ a, 32); \
    c = c + d; \
    b = ROTR64(b ^ c, 24); \
...
#define ARGON2D_QUARD_PERM(a, b, c, d) \
    { \
	uint2 aa = as_uint2(a); \
	uint2 bb = as_uint2(b); \
	uint2 cc = as_uint2(c); \
	...
	__asm ( \
	      "s_nop 1\n" \
		  "v_mov_b32_dpp  %[p10], %[p10] quad_perm:[0,1,2,3]\n" \
	      "v_mov_b32_dpp  %[p11], %[p11] quad_perm:[0,1,2,3]\n" \
		  "v_mov_b32_dpp  %[p20], %[p20] quad_perm:[1,2,3,0]\n" \
		  "v_mov_b32_dpp  %[p21], %[p21] quad_perm:[1,2,3,0]\n" \
		  "v_mov_b32_dpp  %[p30], %[p30] quad_perm:[2,3,0,1]\n" \
		  "v_mov_b32_dpp  %[p31], %[p31] quad_perm:[2,3,0,1]\n" \
          "v_mov_b32_dpp  %[p40], %[p40] quad_perm:[3,0,1,2]\n" \
		  "v_mov_b32_dpp  %[p41], %[p41] quad_perm:[3,0,1,2]\n" \
		  "s_nop 1" \
		  : [p10] "=&v" (aa.x), \
		    [p11] "=&v" (aa.y), \
			[p20] "=&v" (bb.x), \
...
#define ARGON2D_QUARD_PERM_BACK(a, b, c, d) \
    { \
	uint2 aa = as_uint2(a); \
	uint2 bb = as_uint2(b); \
	uint2 cc = as_uint2(c); \
	...
	__asm ( \
		  ...
		  "v_mov_b32_dpp  %[p30], %[p30] quad_perm:[2,3,0,1]\n" \
		  "v_mov_b32_dpp  %[p31], %[p31] quad_perm:[2,3,0,1]\n" \
          "v_mov_b32_dpp  %[p40], %[p40] quad_perm:[1,2,3,0]\n" \
		  "v_mov_b32_dpp  %[p41], %[p41] quad_perm:[1,2,3,0]\n" \
		  "s_nop 1" \
		  : [p10] "=&v" (aa.x), \
		    [p11] "=&v" (aa.y), \
			[p20] "=&v" (bb.x), \
    ...
    uint32_t idx)
{
    ...
    uint64_t a      = state->a;
    uint64_t b      = state->b;
    uint64_t c      = blake2b_IV[idx];
    uint64_t d;
    if(idx==0)
        d = blake2b_IV[4] ^ (step-1)*BLAKE2B_BLOCKBYTES+4;
    else if(idx==2)
    ...
    for (uint32_t r = 0; r < 12; ++r) {
        ...
        ARGON2D_G_SHUFFLE(a, b, c, d, m[ref1], m[ref2]);
        ref1=sigma[r][1]>>16*idx;
        ref2=sigma[r][1]>>(16*idx+8);
        ARGON2D_QUARD_PERM(a,b,c,d);
        ARGON2D_G_SHUFFLE(a, b, c, d, m[ref1], m[ref2]);
        ARGON2D_QUARD_PERM_BACK(a,b,c,d);
    }
    state->a = state->a ^ a ^ c;
    state->b = state->b ^ b ^ d;
}
    ...
    uint32_t idx)
{
    ...
    uint64_t b      = state->b;
    uint64_t c      = blake2b_IV[idx];
    uint64_t d      = blake2b_IV[idx+4]^ counter*BLAKE2B_BLOCKBYTES;
    for (uint32_t r = 0; r < 12; ++r) {
        uint8_t ref1,ref2;
        ref1 = sigma[r][0]>>16*idx;
        ref2 = sigma[r][0]>>(16*idx+8);
...
{
    __local char smem[129 * sizeof(uint64_t) * 8];
    ...
    __local uint64_t* buffer= (__local uint64_t*) 0;//&smem[129*get_local_size(1)*sizeof(uint64_t)+get_local_id(1)*18*sizeof(uint64_t)];
