...
void FFT(const unsigned char *input, swift_int32_t *output, swift_int16_t *mult, swift_int16_t *fftTable)
{
...
void SWIFFTFFT(const unsigned char *input, int m, swift_int32_t *output)
{
	...
	{
		FFT(input, output, multipliers, fftTable);
	}
}
...
void SWIFFTSum(const swift_int32_t *input, int m, unsigned char *output, __constant const swift_int16_t *a)
{
	int i, j;
	swift_int32_t result[NN];
...
void ComputeSingleSWIFFTX(unsigned char input[SWIFFTX_INPUT_BLOCK_SIZE], unsigned char output[SWIFFTX_OUTPUT_BLOCK_SIZE])
{
	int i;
	// Will store the result of the FFT parts:
	swift_int32_t fftOut[NN * MM];
	unsigned char intermediate[NN * 3 + 8];
	...
	SWIFFTFFT(input, MM, fftOut);

	// 2. Compute the sums of the 3 SWIFFTs, each using a different set of coefficients:

	// 2a. The first SWIFFT:
	SWIFFTSum(fftOut, MM, intermediate, As);
	// Remember the carry byte:
	carry0 = intermediate[NN];

	// 2b. The second one:
	SWIFFTSum(fftOut, MM, intermediate + NN, As + (MM * NN));
	carry1 = intermediate[2 * NN];

	// 2c. The third one:
	SWIFFTSum(fftOut, MM, intermediate + (2 * NN), As + 2 * (MM * NN));
	...
	SWIFFTSum(fftOut,       3 * (NN/8) + 1, output, As);
}