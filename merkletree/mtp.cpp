...
void compute_mtp_blake2b(const argon_block& input,
	uint8_t digest[MERKLE_TREE_ELEMENT_SIZE_B])
{
	amtp_blake2b_state state;
	amtp_blake2b_init(&state, MERKLE_TREE_ELEMENT_SIZE_B);
	amtp_blake2b4rounds_update(&state, input.v, MTP_ARGON2_argon_block_SIZE);
	amtp_blake2b4rounds_final(&state, digest, MERKLE_TREE_ELEMENT_SIZE_B);
}
...
void fill_argon_block(__m128i *state, const argon_block *ref_argon_block, argon_block *next_argon_block, int with_xor) {
    __m128i argon_block_XY[MTP_ARGON2_OWORDS_IN_argon_block];
    unsigned int i;

    if (with_xor) {
        for (i = 0; i < MTP_ARGON2_OWORDS_IN_argon_block; i++) {
    ...
    for (i = 0; i < 8; ++i) {
        MTP_BLAKE2_ROUND(state[8 * i + 0], state[8 * i + 1], state[8 * i + 2],
                     state[8 * i + 3], state[8 * i + 4], state[8 * i + 5],
                     state[8 * i + 6], state[8 * i + 7]);
    }

    for (i = 0; i < 8; ++i) {
        MTP_BLAKE2_ROUND(state[8 * 0 + i], state[8 * 1 + i], state[8 * 2 + i],
...
#define TEST_ADLEN 0
    ...
    mtp_argon2_context *pContext = &context;
    ...
    memset(pContext,0,sizeof(mtp_argon2_context));
    ...
    context.version = MTP_ARGON2_VERSION_NUMBER;
...
//		std::string proof_argon_blocks[L * 3];
		...
		amtp_blake2b_init(&BlakeHash, 32);
	...
	for (int i = 0; i<4;i++)
		...
		amtp_blake2b_update(&BlakeHash, (unsigned char*)&input[0], 80);
		amtp_blake2b_update(&BlakeHash, (unsigned char*)&resultMerkleRoot[0], 16);
		amtp_blake2b_update(&BlakeHash, &TheNonce, sizeof(unsigned int));
		amtp_blake2b_final(&BlakeHash, (unsigned char*)&Y[0], 32);
		...
		for (uint8_t j = 1; j <= L; j++) {
			...
			amtp_blake2b_state BlakeHash2;
			amtp_blake2b_init(&BlakeHash2, 32);
			amtp_blake2b_update(&BlakeHash2, &Y[j - 1], sizeof(uint256));
			amtp_blake2b_update(&BlakeHash2, argon_blockhash_bytes, MTP_ARGON2_argon_block_SIZE);
...
// current argon_block
			...
			amtp_blake2b4rounds_update(&state_curr, argon_blockhash_curr_bytes, MTP_ARGON2_argon_block_SIZE);
			...
			amtp_blake2b4rounds_update(&state_prev, argon_blockhash_prev_bytes, MTP_ARGON2_argon_block_SIZE);
			uint8_t digest_prev[MERKLE_TREE_ELEMENT_SIZE_B];


			amtp_blake2b4rounds_final(&state_prev, digest_prev, sizeof(digest_prev));
			...
			amtp_blake2b4rounds_update(&state_ref, argon_blockhash_ref_bytes, MTP_ARGON2_argon_block_SIZE);
...
//			uint512 hashargon_block;
			amtp_blake2b_state ctx;
			amtp_blake2b_init(&ctx, MERKLE_TREE_ELEMENT_SIZE_B);
			amtp_blake2b4rounds_update(&ctx, argon_blockhash_bytes, MTP_ARGON2_argon_block_SIZE);
			uint8_t digest[MERKLE_TREE_ELEMENT_SIZE_B];
			amtp_blake2b4rounds_final(&ctx, digest, sizeof(digest));
...
MerkleTree::Elements   mtp_init2(mtp_argon2_instance_t *instance) {
	...
	if (instance != NULL) {
		...
		for (int i = 0; i < instance->memory_argon_blocks / 2; ++i) {
			...
			compute_mtp_blake2b(instance->memory[2 * i], digest);
			elements.emplace_back(digest, digest + sizeof(digest));
			memset(digest, 0, MERKLE_TREE_ELEMENT_SIZE_B);
			compute_mtp_blake2b(instance->memory[2 * i + 1], digest);
			elements.emplace_back(digest, digest + sizeof(digest));
...
void mtp_hash(char* output, const char* input, unsigned int d,uint32_t TheNonce) {
    mtp_argon2_context context = init_mtp_argon2d_param(input);
